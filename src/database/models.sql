-- Habesha Dating Bot - Supabase Database Schema
...
DECLARE
    ...
    v_looking_for TEXT;
BEGIN
    -- Get user's gender and what they're looking for
    SELECT gender, looking_for INTO v_gender, v_looking_for
    ...
    SELECT 
        ...
        CASE 
            WHEN u.religion = (SELECT religion FROM users WHERE id = p_user_id) THEN 30
            ELSE 0
        END +
        CASE 
            WHEN u.region = (SELECT region FROM users WHERE id = p_user_id) THEN 20
            ELSE 0
        END +
        CASE 
            WHEN u.education = (SELECT education FROM users WHERE id = p_user_id) THEN 15
            ELSE 0
        END +
        CASE 
            WHEN ABS(u.age - (SELECT age FROM users WHERE id = p_user_id)) <= 5 THEN 25
            WHEN ABS(u.age - (SELECT age FROM users WHERE id = p_user_id)) <= 10 THEN 10
            ELSE 0
        END as compatibility_score
    FROM users u
    WHERE 
        u.id != p_user_id
        AND u.gender = v_looking_for
        AND u.status = 'active'
